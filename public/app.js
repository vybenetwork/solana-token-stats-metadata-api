...
fetchAllBtn.addEventListener('click', async () => {
  ...
  try {
    // Top holders first, then 2s delay, then token details (to space out requests)
    ...
    if (holdersRes.ok && holdersData.data?.length) {
      ...
      holdersSection.hidden = false;
    } else {
      holdersSection.hidden = true;
    }
...
function renderHolders(data) {
  ...
  holdersMeta.textContent = `Top 100 holders sorted by highest % of supply (${list.length} shown; updated every 3 hours).`;

  holdersBody.innerHTML = list
    .map((h) => {
      const rank = h.rank ?? '—';
      const owner = h.ownerName || h.ownerAddress || '—';
      ...
      return `<tr>
        <td>${rank}</td>
        <td><span class="mono" title="${h.ownerAddress || ''}">${owner}</span></td>
        <td>${balance}</td>
        <td>${valueUsd}</td>
        <td>${pct}</td>
      </tr>`;
