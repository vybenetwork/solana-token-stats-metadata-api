...
function renderTradesSummary(opts: {
  ...
  const programRows = uniquePrograms
    ...
    .map((addr) => {
      ...
      const link = `<a href="${SOLSCAN_ACCOUNT}${encodeURIComponent(addr)}" target="_blank" class="mono" title="${addr}">${truncateProgramAddress(addr)}</a>`;
      const markets = programTopMarkets[addr] || [];
      const top = markets.find((m) => !m.bestQuoteMint || hasQuoteSymbol(m.bestQuoteMint, quoteSymbols))!;
      const marketLink = `<a href="${SOLSCAN_ACCOUNT}${encodeURIComponent(top.marketAddress)}" target="_blank" class="mono" title="${top.marketAddress}">${truncateAddress(top.marketAddress)}</a>`;
  ...
  const quoteRows = top10QuoteMints
    .map(({ mint, count }) => {
      const mintLink = `<a href="${VYBE_TOKEN}${encodeURIComponent(mint)}" target="_blank" class="mono" title="${mint}">${truncateAddress(mint)}</a>`;
  ...
  const marketRows = top10Markets
    .map(({ marketAddress, count, bestQuoteMint }) => {
      const marketLink = `<a href="${SOLSCAN_ACCOUNT}${encodeURIComponent(marketAddress)}" target="_blank" class="mono" title="${marketAddress}">${truncateAddress(marketAddress)}</a>`;
      const baseSymDisplay = truncateSymbolDisplay(baseSymbol);
    ...
    ? list
        .map((row, i) => {
          ...
          const accountLink = addr
            ? `<a href="${VYBE_WALLET}${encodeURIComponent(addr)}" target="_blank" class="mono" title="${addr}">${display}</a>`
    ...
    ? list
        .map((h) => {
          ...
          const ownerLink = h.ownerAddress
            ? `<a href="${VYBE_WALLET}${encodeURIComponent(h.ownerAddress)}" target="_blank" class="mono" title="${h.ownerAddress}">${ownerDisplay}</a>`
